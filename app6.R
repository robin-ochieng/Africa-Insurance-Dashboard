...
server <- function(input, output, session) {
  ...
  output$map <- renderPlotly({
    ...
    # Join with world map data, focusing on Africa by geographical bounds
    world <- map_data("world")
    # Filter for Africa based on rough geographical bounds
    africa <- world %>%
      filter(long > -20, long < 60, lat > -35, lat < 40)
    map_data <- merge(africa, aggregated_data, by.x = "region", by.y = "Country", all.x = TRUE)
    
    # Generate the heatmap focused on Africa
    p <- ggplot(map_data, aes(x = long, y = lat, group = group, fill = Total_GWP)) +
      ...
      theme_void() +
      xlim(-20, 60) + ylim(-35, 40)  # Adjust these limits to better fit Africa's geographical bounds
