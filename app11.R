...
ui <- bs4DashPage(
  ...
  header = dashboardHeader(
    ...
    title = dashboardBrand(
      title = "Life Insurers In Africa Report",
      color = "primary"
    ),
    controlbarIcon = NULL
  ),
...
server <- function(input, output, session) {
  ...
  output$barPlot <- renderPlotly({
    ...
    p <- ggplot(filteredData(), aes(x = Insurer, y = Gross_written_premium_in_USD)) +
      ...
      geom_text(aes(label = paste0(sprintf("%.2f", Gross_written_premium_in_USD / 1e6), "M"),
                    vjust = dynamic_vjust(Gross_written_premium_in_USD)), color = "black", size = 2.5) +
      theme_minimal() +
      theme(
  ...
  output$map <- renderLeaflet({
    ...
    aggregated_data <- life_insurers %>%
      ...
      # Convert Total_GWP to millions with 2 decimal points and add "M" suffix
  ...
  output$numImplementedBox <- renderValueBox({
    valueBox(
      ...
      subtitle = "Percentage of Countries Implemented IFRS17",
      color = "primary",
