...
server <- function(input, output, session) {
  ...
  output$barPlot <- renderPlotly({
    p <- ggplot(filteredData(), aes(x = Life_Insurers, y = Gross_written_premium_in_USD)) +
      ...
      geom_text(aes(label = Gross_written_premium_in_USD), vjust = -0.3, color = "black") + # Value labels
  ...
  output$scatterPlot <- renderPlotly({
    p <- ggplot(filteredData(), aes(x = Gross_written_premium_in_USD, y = Life_Insurers, color = Country)) +
      geom_point(size = 3, shape = 18) + # Different shape for better visibility
      scale_color_viridis_d() + # A sophisticated color palette
      theme_minimal() + # Cleaner theme
